...
function scan(markup: string, style = '', config = {}) {
  ...
  return collectFindings(imageryRules, createScanContext(window, config));
}

function ruleFindings(ruleId: string, markup: string, style = '', config = {}) {
...
describe('imagery lint rules through the real engine', () => {
  ...
  describe('shape-assembled-illustration', () => {
    it('fires on a large inline svg built from 8 primitives and 3 fills', () => {
    ...
    it('does not fire under the size, primitive, fill, text or pattern thresholds', () => {
      expect(ruleFindings('shape-assembled-illustration', `<svg width="199" height="300">${SCENE}</svg>`)).toEqual([]);
      expect(ruleFindings('shape-assembled-illustration', `<svg viewBox="0 0 400 300">${SCENE.replace('<ellipse/>', '')}</svg>`)).toEqual([]);
      expect(ruleFindings('shape-assembled-illustration', `<svg viewBox="0 0 400 300">${SCENE.replace('#0f0', 'currentColor')}</svg>`)).toEqual([]);
      expect(ruleFindings('shape-assembled-illustration', `<svg viewBox="0 0 400 300">${SCENE}<text>a</text><text>b</text><tspan>c</tspan></svg>`)).toEqual([]);
      expect(ruleFindings('shape-assembled-illustration', `<svg viewBox="0 0 400 300"><defs><pattern id="p"></pattern></defs>${SCENE}</svg>`)).toEqual([]);
    });

    it('keeps stroke-width out of the width lookup (a width attr wins over viewBox)', () => {
      expect(ruleFindings('shape-assembled-illustration', `<svg stroke-width="1" width="100" viewBox="0 0 400 300">${SCENE}</svg>`)).toEqual([]);
      expect(ruleFindings('shape-assembled-illustration', `<svg viewBox="0 0 400 300">${SCENE}<text>a</text><text>b</text></svg>`)).toHaveLength(1);
    });
  });
  ...
  describe('organic-clip-path', () => {
    ...
    it('fires on a 10-vertex polygon with every coordinate off the 25% grid', () => {
      const findings = ruleFindings('organic-clip-path', '<div class="blob"></div>', `.blob { clip-path: polygon(${offGrid}); }`);
      expect(findings).toHaveLength(1);
      expect(findings[0]).toMatchObject({
        severity: 'warning',
    ...
    it('fires once off-grid numbers reach the vertex count, even with on-grid coordinates', () => {
      const tenOffGrid = offGridXs.map((x) => `${x}% 0%`).join(', ');
      const findings = ruleFindings('organic-clip-path', '<div></div>', `.a { clip-path: polygon(${tenOffGrid}); }`);
      expect(findings.map((finding) => finding.detail)).toEqual(['clip-path: polygon() with 10 vertices approximating an organic contour']);
    });

    it('fires on a path() with 3 curve commands in an inline style attribute', () => {
      const findings = ruleFindings('organic-clip-path', `<div id="p" style="clip-path: path('M0 0 C 1 2 3 4 5 6 S 1 1 2 2 Q 1 1 3 3 Z')"></div>`);
      expect(findings).toHaveLength(1);
      expect(findings[0]!.detail).toBe('clip-path: path() with 3 curve segments');
      expect(findings[0]!.el?.id).toBe('p');
    });

    it('does not fire at 9 vertices, on-grid vertices, or a path with 2 curves', () => {
      ...
      expect(ruleFindings('organic-clip-path', '<div></div>', `.a { clip-path: polygon(${nine}); }`)).toEqual([]);
      expect(ruleFindings('organic-clip-path', '<div></div>', `.a { clip-path: polygon(${onGrid}); }`)).toEqual([]);
      expect(ruleFindings('organic-clip-path', '<div></div>', `.a { clip-path: polygon(${nineOffGrid}); }`)).toEqual([]);
      expect(ruleFindings('organic-clip-path', `<div style="clip-path: path('M0 0 C 1 2 3 4 5 6 Q 1 1 3 3 L 1 1 Z')"></div>`)).toEqual([]);
    });
  });

  describe('buried-raster', () => {
    it('fires on an <img> at computed opacity under 0.15', () => {
      const findings = ruleFindings('buried-raster', '<img src="a.png" alt="Hero" style="opacity: 0.1">');
      expect(findings).toHaveLength(1);
      expect(findings[0]).toMatchObject({ detail: '<img> at opacity 0.1 "Hero"' });
      expect(findings[0]!.el?.localName).toBe('img');
    });

    it('fires on a background url at near-zero opacity and labels it with its text', () => {
      const findings = ruleFindings('buried-raster', `<div style="opacity: 0; background-image: url('t.png')">Texture</div>`);
      expect(findings.map((finding) => finding.detail)).toEqual(['raster background at opacity 0 "Texture"']);
    });

    it('does not fire at opacity 0.15 or on a non-raster element at low opacity', () => {
      expect(ruleFindings('buried-raster', '<img src="a.png" style="opacity: 0.15">')).toEqual([]);
      expect(ruleFindings('buried-raster', '<div style="opacity: 0.05">text</div>')).toEqual([]);
    });

    it('fires on a url() under a gradient wash whose stops are all at least 0.9 alpha', () => {
      const findings = ruleFindings(
        'buried-raster',
    ...
    it('does not fire on a sub-0.9 tint, a gradient after the url, or a declared blend mode', () => {
      expect(ruleFindings('buried-raster', '<p></p>', '.a { background: linear-gradient(rgba(0, 0, 0, 0.89), black), url(p.jpg); }')).toEqual([]);
      expect(ruleFindings('buried-raster', '<p></p>', '.a { background: url(p.jpg), linear-gradient(black, black); }')).toEqual([]);
      expect(ruleFindings('buried-raster', '<p></p>', '.a { background: linear-gradient(black, black), url(p.jpg); background-blend-mode: multiply; }')).toEqual([]);
    });
  });

  describe('broken-image', () => {
    it('fires on an <img> with no src, an empty src and a # placeholder', () => {
      const findings = ruleFindings('broken-image', '<img alt="a"><img src=""><img src=" # ">');
    ...
    it('does not fire on a real src, a fragment src, or a non-img element', () => {
      expect(ruleFindings('broken-image', '<img src="a.png"><img src="#hero"><video></video><div src=""></div>')).toEqual([]);
    });
  });
});
